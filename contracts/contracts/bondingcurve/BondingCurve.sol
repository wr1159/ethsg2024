...
import { Timed } from "../helpers/Timed.sol";
...
abstract contract BondingCurve is
	IBondingCurve,
	Initializable,
	Pausable,
	Ownable2Step,
	Timed
{
		...
		returns (UD60x18 amountOut)
	{
		...
		acceptedToken.transferFrom(from, address(this), amountIn);
		token.transfer(to, unwrap(amountOut));
	...
	function allocate(
		uint256 amount,
		address to
	) external virtual override onlyOwner afterTime {
	...
	function getCurrentPrice(
		UD60x18 tokenSupply
	) external view virtual returns (UD60x18);
	...
	) internal returns (UD60x18 saleTokenAmountOut) {
		saleTokenAmountOut = calculatePurchaseAmountOut(ud(tokenAmountIn));

		require(
			gte(availableToSell(), saleTokenAmountOut),
			"BondingCurve: exceeds cap"
		);
